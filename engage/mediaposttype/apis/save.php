...
/**
 * crm/engage/mediaposttype/apis/save.php
 *
 * ====
 * Save
 * ====
 * Menampilkan satu baris data/record sesuai PrimaryKey,
 * dari tabel header mediaposttype (mst_mediaposttype)
 *
 ...
 * tanggal 23/11/2023
 */
$API = new class extends mediaposttypeBase {
	
	public function execute($data, $options) {
		$event = 'on-save';
		$tablename = 'mst_mediaposttype';
		$primarykey = 'mediaposttype_id';
		$autoid = $options->autoid;
		$datastate = $data->_state;
		$userdata = $this->auth->session_get_user();

		$handlerclassname = "\\FGTA4\\apis\\mediaposttype_headerHandler";
		$hnd = null;
		if (class_exists($handlerclassname)) {
			$hnd = new mediaposttype_headerHandler($options);
			$hnd->caller = &$this;
			$hnd->db = &$this->db;
			$hnd->auth = $this->auth;
		...
		try {
			...
			$obj->mediaposttype_id = strtoupper($obj->mediaposttype_id);
			$obj->mediaposttype_name = strtoupper($obj->mediaposttype_name);
			...
			try {
				...
				// result
				$options->criteria = [
					"mediaposttype_id" => $obj->mediaposttype_id
				];

				$criteriaValues = [
					"mediaposttype_id" => " mediaposttype_id = :mediaposttype_id "
				];
				...
				$sqlFieldList = [
					'mediaposttype_id' => 'A.`mediaposttype_id`', 'mediaposttype_name' => 'A.`mediaposttype_name`', 'mediaposttype_iskol' => 'A.`mediaposttype_iskol`', '_createby' => 'A.`_createby`',
					'_createby' => 'A.`_createby`', '_createdate' => 'A.`_createdate`', '_modifyby' => 'A.`_modifyby`', '_modifydate' => 'A.`_modifydate`'
				];
				$sqlFromTable = "mst_mediaposttype A";
				$sqlWhere = $where->sql;
