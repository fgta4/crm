...
import * as hnd from  './crmevent-invitedgrid-hnd.mjs'

const tbl_list = $('#pnl_editinvitedgrid-tbl_list');
const txt_title = $('#pnl_editinvitedgrid-title');
const pnl_control = $('.pnl_editinvitedgrid-control');
const btn_removechecked  = $('#pnl_editinvitedgrid-removechecked');
const btn_addrow = $('#pnl_editinvitedgrid-addrow');
...
export function OpenDetil(data) {
	...
	var fn_listloading = async (options) => {
		options.api = `${global.modulefullname}/invited-list`
	...
	var fn_listloaded = async (result, options) => {
		...
		var detilform = $ui.getPages().ITEMS['pnl_editinvitedform'].handler.getForm()
		...
		setTimeout(()=>{
			var checkcolumns = document.querySelectorAll('#pnl_editinvitedgrid-tbl_list .rowcheck');
			...
			var selectbutton = document.getElementById('pnl_editinvitedgrid-tbl_list-selectall-button');
			if (detilform.AllowRemoveRecord) {
...
function grd_list_rowclick(tr, ev) {
	...
	$ui.getPages().show('pnl_editinvitedform', () => {
		$ui.getPages().ITEMS['pnl_editinvitedform'].handler.open(record, trid, header_data)
	})	
}
		...
		yes: () => {
			grd_list.removechecked({
				OnRemoving : async (options) => {
					var apiurl = `${global.modulefullname}/invited-delete`
...
function btn_addrow_click() {
	$ui.getPages().show('pnl_editinvitedform', ()=>{
		$ui.getPages().ITEMS['pnl_editinvitedform'].handler.createnew(header_data)
	})	
}