...
import * as hnd from  './crmevent-postgrid-hnd.mjs'

const tbl_list = $('#pnl_editpostgrid-tbl_list');
const txt_title = $('#pnl_editpostgrid-title');
const pnl_control = $('.pnl_editpostgrid-control');
const btn_removechecked  = $('#pnl_editpostgrid-removechecked');
const btn_addrow = $('#pnl_editpostgrid-addrow');
...
export function OpenDetil(data) {
	...
	var fn_listloading = async (options) => {
		options.api = `${global.modulefullname}/post-list`
	...
	var fn_listloaded = async (result, options) => {
		...
		var detilform = $ui.getPages().ITEMS['pnl_editpostform'].handler.getForm()
		...
		setTimeout(()=>{
			var checkcolumns = document.querySelectorAll('#pnl_editpostgrid-tbl_list .rowcheck');
			...
			var selectbutton = document.getElementById('pnl_editpostgrid-tbl_list-selectall-button');
			if (detilform.AllowRemoveRecord) {
...
function grd_list_rowclick(tr, ev) {
	...
	$ui.getPages().show('pnl_editpostform', () => {
		$ui.getPages().ITEMS['pnl_editpostform'].handler.open(record, trid, header_data)
	})	
}
		...
		yes: () => {
			grd_list.removechecked({
				OnRemoving : async (options) => {
					var apiurl = `${global.modulefullname}/post-delete`
...
function btn_addrow_click() {
	$ui.getPages().show('pnl_editpostform', ()=>{
		$ui.getPages().ITEMS['pnl_editpostform'].handler.createnew(header_data)
	})	
}