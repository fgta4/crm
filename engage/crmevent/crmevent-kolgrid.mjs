...
import * as hnd from  './crmevent-kolgrid-hnd.mjs'

const tbl_list = $('#pnl_editkolgrid-tbl_list');
const txt_title = $('#pnl_editkolgrid-title');
const pnl_control = $('.pnl_editkolgrid-control');
const btn_removechecked  = $('#pnl_editkolgrid-removechecked');
const btn_addrow = $('#pnl_editkolgrid-addrow');
...
export function OpenDetil(data) {
	...
	var fn_listloading = async (options) => {
		options.api = `${global.modulefullname}/kol-list`
	...
	var fn_listloaded = async (result, options) => {
		...
		var detilform = $ui.getPages().ITEMS['pnl_editkolform'].handler.getForm()
		...
		setTimeout(()=>{
			var checkcolumns = document.querySelectorAll('#pnl_editkolgrid-tbl_list .rowcheck');
			...
			var selectbutton = document.getElementById('pnl_editkolgrid-tbl_list-selectall-button');
			if (detilform.AllowRemoveRecord) {
...
function grd_list_rowclick(tr, ev) {
	...
	$ui.getPages().show('pnl_editkolform', () => {
		$ui.getPages().ITEMS['pnl_editkolform'].handler.open(record, trid, header_data)
	})	
}
		...
		yes: () => {
			grd_list.removechecked({
				OnRemoving : async (options) => {
					var apiurl = `${global.modulefullname}/kol-delete`
...
function btn_addrow_click() {
	$ui.getPages().show('pnl_editkolform', ()=>{
		$ui.getPages().ITEMS['pnl_editkolform'].handler.createnew(header_data)
	})	
}