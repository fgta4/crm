...
/**
 * crm/engage/crmevent/apis/attendant-save.php
 *
 * ==========
 * Detil-Save
 * ==========
...
$API = new class extends crmeventBase {
	...
	public function execute($data, $options) {
		...
		if (class_exists($handlerclassname)) {
			$hnd = new crmevent_attendantHandler($data, $options);
			$hnd->caller = &$this;
			$hnd->db = &$this->db;
			$hnd->auth = $this->auth;
		...
		try {
			...
			$sqlUpdateField  = [
					'crmeventattendant_id', 'invitation_id', 'crmeventattendant_phone', 'crmeventattendant_name',
					'crmeventattendant_address', 'crmeventattendant_city', 'crmeventattendant_date', 'crmeventattendant_time',
					'crmevent_id'
			];
			...
			$obj->crmeventattendant_date = (\DateTime::createFromFormat('d/m/Y',$obj->crmeventattendant_date))->format('Y-m-d');

			$obj->crmeventattendant_phone = strtoupper($obj->crmeventattendant_phone);
			$obj->crmeventattendant_name = strtoupper($obj->crmeventattendant_name);
			$obj->crmeventattendant_address = strtoupper($obj->crmeventattendant_address);
			$obj->crmeventattendant_city = strtoupper($obj->crmeventattendant_city);


			if ($obj->crmeventattendant_date=='') { $obj->crmeventattendant_date = '--NULL--'; }
			if ($obj->crmeventattendant_time=='') { $obj->crmeventattendant_time = '--NULL--'; }
			...
			try {
				...
				// result
				$options->criteria = [
					"crmeventattendant_id" => $obj->crmeventattendant_id
				];
				...
				$dataresponse = array_merge($record, [
					...
					'crmeventattendant_date' => date("d/m/Y", strtotime($row['crmeventattendant_date'])),
