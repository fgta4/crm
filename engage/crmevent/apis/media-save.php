...
if (is_file(__DIR__ .'/data-media-handler.php')) {
	require_once __DIR__ .'/data-media-handler.php';
}
...
/**
 * crm/engage/crmevent/apis/media-save.php
 *
 * ==========
 * Detil-Save
 * ==========
 * Menampilkan satu baris data/record sesuai PrimaryKey,
 * dari tabel media crmevent (trn_crmeventmedia)
 *
...
$API = new class extends crmeventBase {
	...
	public function execute($data, $options, $files) {
		$event = 'on-save';
		$tablename = 'trn_crmeventmedia';
		$primarykey = 'crmeventmedia_id';
		$autoid = $options->autoid;
		$datastate = $data->_state;

		$userdata = $this->auth->session_get_user();

		$handlerclassname = "\\FGTA4\\apis\\crmevent_mediaHandler";
		if (class_exists($handlerclassname)) {
			$hnd = new crmevent_mediaHandler($data, $options);
			$hnd->caller = &$this;
			$hnd->db = &$this->db;
			$hnd->auth = $this->auth;
		...
		try {
			...
			$sqlUpdateField  = [
					'crmeventmedia_id', 'media_id', 'crmeventmedia_cost', 'crmeventmedia_notes',
					'crmeventmedia_file', 'crmevent_id'
			];
			...
			try {
				...
				if (property_exists($files, $fieldname)) {
					...
					$objfile->crmeventmedia_file = $rev;
				...
				// result
				$options->criteria = [
					"crmeventmedia_id" => $obj->crmeventmedia_id
				];

				$criteriaValues = [
					"crmeventmedia_id" => " crmeventmedia_id = :crmeventmedia_id "
				];
				...
				$sqlFieldList = [
					'crmeventmedia_id' => 'A.`crmeventmedia_id`', 'media_id' => 'A.`media_id`', 'crmeventmedia_cost' => 'A.`crmeventmedia_cost`', 'crmeventmedia_notes' => 'A.`crmeventmedia_notes`',
					'crmeventmedia_file' => 'A.`crmeventmedia_file`', 'crmevent_id' => 'A.`crmevent_id`', '_createby' => 'A.`_createby`', '_createdate' => 'A.`_createdate`',
					'_createby' => 'A.`_createby`', '_createdate' => 'A.`_createdate`', '_modifyby' => 'A.`_modifyby`', '_modifydate' => 'A.`_modifydate`'
				];
				$sqlFromTable = "trn_crmeventmedia A";
				$sqlWhere = $where->sql;
