...
/**
 * crm/engage/crmevent/apis/post-save.php
 *
 * ==========
 * Detil-Save
 * ==========
...
$API = new class extends crmeventBase {
	...
	public function execute($data, $options, $files) {
		...
		if (class_exists($handlerclassname)) {
			$hnd = new crmevent_postHandler($data, $options);
			$hnd->caller = &$this;
			$hnd->db = &$this->db;
			$hnd->auth = $this->auth;
		...
		try {
			...
			$sqlUpdateField  = [
					'crmeventpost_id', 'crmeventpost_url', 'crmeventpost_dtpost', 'crmeventpost_dtreported',
					'mediaposttype_id', 'kol_id', 'media_id', 'crmeventpost_impression',
					'crmeventpost_reach', 'crmeventpost_view', 'crmeventpost_comment', 'crmeventpost_likes',
					'crmeventpost_share', 'crmeventpost_save', 'crmeventpost_engagementrate', 'mediaposttype_iskol',
					'crmevent_id'
			];
			...
			$obj->crmeventpost_dtpost = (\DateTime::createFromFormat('d/m/Y',$obj->crmeventpost_dtpost))->format('Y-m-d');
			$obj->crmeventpost_dtreported = (\DateTime::createFromFormat('d/m/Y',$obj->crmeventpost_dtreported))->format('Y-m-d');

			$obj->crmeventpost_url = strtolower($obj->crmeventpost_url);


			if ($obj->crmeventpost_url=='') { $obj->crmeventpost_url = '--NULL--'; }
			...
			try {
				...
				// result
				$options->criteria = [
					"crmeventpost_id" => $obj->crmeventpost_id
				];
				...
				$dataresponse = array_merge($record, [
					...
					'crmeventpost_dtpost' => date("d/m/Y", strtotime($row['crmeventpost_dtpost'])),
					'crmeventpost_dtreported' => date("d/m/Y", strtotime($row['crmeventpost_dtreported'])),
