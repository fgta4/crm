...
/**
 * crm/engage/crmevent/apis/invited-save.php
 *
 * ==========
 * Detil-Save
 * ==========
...
$API = new class extends crmeventBase {
	...
	public function execute($data, $options) {
		...
		if (class_exists($handlerclassname)) {
			$hnd = new crmevent_invitedHandler($data, $options);
			$hnd->caller = &$this;
			$hnd->db = &$this->db;
			$hnd->auth = $this->auth;
		...
		try {
			...
			$sqlUpdateField  = [
					'crmeventinvited_id', 'crmeventinvited_name', 'crmeventinvited_address', 'crmeventinvited_city',
					'crmeventinvited_phone', 'crmeventinvited_email', 'invitation_id', 'crmeventinvited_iscontacted',
					'crmeventinvited_contactdate', 'user_id', 'crmevent_id'
			];
			...
			$obj->crmeventinvited_contactdate = (\DateTime::createFromFormat('d/m/Y',$obj->crmeventinvited_contactdate))->format('Y-m-d');

			$obj->crmeventinvited_name = strtoupper($obj->crmeventinvited_name);
			$obj->crmeventinvited_address = strtoupper($obj->crmeventinvited_address);
			$obj->crmeventinvited_city = strtoupper($obj->crmeventinvited_city);
			$obj->crmeventinvited_phone = strtoupper($obj->crmeventinvited_phone);
			$obj->crmeventinvited_email = strtolower($obj->crmeventinvited_email);
			$obj->user_id = strtoupper($obj->user_id);


			if ($obj->crmeventinvited_address=='') { $obj->crmeventinvited_address = '--NULL--'; }
			if ($obj->crmeventinvited_city=='') { $obj->crmeventinvited_city = '--NULL--'; }
			if ($obj->crmeventinvited_phone=='') { $obj->crmeventinvited_phone = '--NULL--'; }
			if ($obj->crmeventinvited_email=='') { $obj->crmeventinvited_email = '--NULL--'; }
			if ($obj->invitation_id=='') { $obj->invitation_id = '--NULL--'; }
			if ($obj->crmeventinvited_contactdate=='') { $obj->crmeventinvited_contactdate = '--NULL--'; }
			...
			try {
				...
				// result
				$options->criteria = [
					"crmeventinvited_id" => $obj->crmeventinvited_id
				];
				...
				$dataresponse = array_merge($record, [
					...
					'crmeventinvited_contactdate' => date("d/m/Y", strtotime($row['crmeventinvited_contactdate'])),
