...
import * as hnd from  './crmevent-attendantgrid-hnd.mjs'
...
export function OpenDetil(data) {
	...
	var fn_listloaded = async (result, options) => {
		...
		setTimeout(()=>{
			var checkcolumns = document.querySelectorAll('#pnl_editattendantgrid-tbl_list .rowcheck');
			...
			var selectbutton = document.getElementById('pnl_editattendantgrid-tbl_list-selectall-button');
			if (detilform.AllowRemoveRecord) {
