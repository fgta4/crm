...
import * as hnd from  './crmevent-mediagrid-hnd.mjs'

const tbl_list = $('#pnl_editmediagrid-tbl_list');
const txt_title = $('#pnl_editmediagrid-title');
const pnl_control = $('.pnl_editmediagrid-control');
const btn_removechecked  = $('#pnl_editmediagrid-removechecked');
const btn_addrow = $('#pnl_editmediagrid-addrow');
...
export function OpenDetil(data) {
	...
	var fn_listloading = async (options) => {
		options.api = `${global.modulefullname}/media-list`
	...
	var fn_listloaded = async (result, options) => {
		...
		var detilform = $ui.getPages().ITEMS['pnl_editmediaform'].handler.getForm()
		...
		setTimeout(()=>{
			var checkcolumns = document.querySelectorAll('#pnl_editmediagrid-tbl_list .rowcheck');
			...
			var selectbutton = document.getElementById('pnl_editmediagrid-tbl_list-selectall-button');
			if (detilform.AllowRemoveRecord) {
...
function grd_list_rowclick(tr, ev) {
	...
	$ui.getPages().show('pnl_editmediaform', () => {
		$ui.getPages().ITEMS['pnl_editmediaform'].handler.open(record, trid, header_data)
	})	
}
		...
		yes: () => {
			grd_list.removechecked({
				OnRemoving : async (options) => {
					var apiurl = `${global.modulefullname}/media-delete`
...
function btn_addrow_click() {
	$ui.getPages().show('pnl_editmediaform', ()=>{
		$ui.getPages().ITEMS['pnl_editmediaform'].handler.createnew(header_data)
	})	
}