...
/**
 * crm/engage/koltype/apis/save.php
 *
 * ====
 * Save
 * ====
 * Menampilkan satu baris data/record sesuai PrimaryKey,
 * dari tabel header koltype (mst_koltype)
 *
 ...
 * tanggal 24/11/2023
 */
$API = new class extends koltypeBase {
	
	public function execute($data, $options) {
		$event = 'on-save';
		$tablename = 'mst_koltype';
		$primarykey = 'koltype_id';
		$autoid = $options->autoid;
		$datastate = $data->_state;
		$userdata = $this->auth->session_get_user();

		$handlerclassname = "\\FGTA4\\apis\\koltype_headerHandler";
		$hnd = null;
		if (class_exists($handlerclassname)) {
			$hnd = new koltype_headerHandler($options);
			$hnd->caller = &$this;
			$hnd->db = &$this->db;
			$hnd->auth = $this->auth;
		...
		try {
			...
			$obj->koltype_name = strtoupper($obj->koltype_name);
			...
			try {
				...
				// result
				$options->criteria = [
					"koltype_id" => $obj->koltype_id
				];

				$criteriaValues = [
					"koltype_id" => " koltype_id = :koltype_id "
				];
				...
				$sqlFieldList = [
					'koltype_id' => 'A.`koltype_id`', 'koltype_name' => 'A.`koltype_name`', '_createby' => 'A.`_createby`', '_createdate' => 'A.`_createdate`',
					'_createby' => 'A.`_createby`', '_createdate' => 'A.`_createdate`', '_modifyby' => 'A.`_modifyby`', '_modifydate' => 'A.`_modifydate`'
				];
				$sqlFromTable = "mst_koltype A";
				$sqlWhere = $where->sql;
