...
import * as hnd from  './broadcastmsg-resultgrid-hnd.mjs'

const tbl_list = $('#pnl_editresultgrid-tbl_list');
const txt_title = $('#pnl_editresultgrid-title');
const pnl_control = $('.pnl_editresultgrid-control');
const btn_removechecked  = $('#pnl_editresultgrid-removechecked');
const btn_addrow = $('#pnl_editresultgrid-addrow');
...
export function createnew(data, options) {
	...
	grd_list.clear();
	txt_title.html(data.broadcastmsg_descr)
	header_data = data;
}
...
export function OpenDetil(data) {
	...
	var fn_listloading = async (options) => {
		options.api = `${global.modulefullname}/result-list`
		options.criteria['id'] = data.broadcastmsg_id
	}
	var fn_listloaded = async (result, options) => {
		...
		var detilform = $ui.getPages().ITEMS['pnl_editresultform'].handler.getForm()
...
function grd_list_rowclick(tr, ev) {
	...
	$ui.getPages().show('pnl_editresultform', () => {
		$ui.getPages().ITEMS['pnl_editresultform'].handler.open(record, trid, header_data)
	})	
}
		...
		yes: () => {
			grd_list.removechecked({
				OnRemoving : async (options) => {
					var apiurl = `${global.modulefullname}/result-delete`
...
function btn_addrow_click() {
	$ui.getPages().show('pnl_editresultform', ()=>{
		$ui.getPages().ITEMS['pnl_editresultform'].handler.createnew(header_data)
	})	
}