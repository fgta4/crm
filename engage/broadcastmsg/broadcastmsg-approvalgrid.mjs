...
export function createnew(data, options) {
	...
	grd_list.clear();
	txt_title.html(data.broadcastmsg_id)
	header_data = data;
}
...
function grd_list_rowrender(tr) {
	...
	$(tr).find('td').each((i, td) => {
		...
		if (mapping=='docauth_descr') {
			...
			var authby = "";
			var notes = "";			
			if (record.broadcastmsgappr_isdeclined=="1") {
				gambar = `<img src="images/xtiondecl.png" width="20px" height="20px">`;
				authby = `<div><span style="font-weight: bold">${record.broadcastmsgappr_declinedby}</span> (${record.broadcastmsgappr_declineddate})</div>`
				notes = record.broadcastmsgappr_notes.replace(/(?:\r\n|\r|\n)/g, '<br>');
				notes = `<div style="margin-top: 5px; font-style: italic">${notes}</div>`
			} else if (record.broadcastmsgappr_isapproved=="1") {
				gambar = `<img src="images/xtionappr.png" width="20px" height="20px">`;
				authby = `<div><span style="font-weight: bold">${record.broadcastmsgappr_by}</span> (${record.broadcastmsgappr_date})</div>`
			} else {
