...
/**
 * crm/engage/broadcastmsg/apis/files-save.php
 *
 * ==========
 * Detil-Save
 * ==========
	...
	public function execute($data, $options, $files) {
		...
		try {
			...
			try {
				...
				$fieldname = 'broadcastmsgfiles_file';	
				if (property_exists($files, $fieldname)) {
					...
					$obj->broadcastmsgfiles_file = $rev;
					$cmd = \FGTA4\utils\SqlUtility::CreateSQLUpdate($tablename, $obj, $key);
					$stmt = $this->db->prepare($cmd->sql);
					$stmt->execute($cmd->params);
				}				
				...
				$sql = \FGTA4\utils\SqlUtility::Select($tablename , [
					$primarykey
					, 'broadcastmsgfiles_id', 'doctype_id', 'broadcastmsgfiles_descr', 'broadcastmsgfiles_order', 'broadcastmsgfiles_file', 'broadcastmsg_id', '_createby', '_createdate', '_modifyby', '_modifydate', '_createby', '_createdate', '_modifyby', '_modifydate'
				], $where->sql);
