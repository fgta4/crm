	...
	public function execute($options) {
		...
		try {
			...
			$where = \FGTA4\utils\SqlUtility::BuildCriteria(
				$options->criteria,
				[
					"crmmsgcstx_id" => " crmmsgcstx_id = :crmmsgcstx_id "
				]
			);

			$sql = \FGTA4\utils\SqlUtility::Select('trn_crmmsgcstx', [
				'crmmsgcstx_id', 'crmmsgcstx_date', 'crmmsgcstx_value', '_createby', '_createdate', '_modifyby', '_modifydate' 
			], $where->sql);
			...
			$result->record = array_merge($record, [
				'crmmsgcstx_date' => date("d/m/Y", strtotime($record['crmmsgcstx_date'])),
