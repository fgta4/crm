	...
	public function execute($options) {
		...
		try {
			...
			$where = \FGTA4\utils\SqlUtility::BuildCriteria(
				$options->criteria,
				[
					"crmmsgcsitem_id" => " crmmsgcsitem_id = :crmmsgcsitem_id "
				]
			);

			$sql = \FGTA4\utils\SqlUtility::Select('trn_crmmsgcsitem', [
				'crmmsgcsitem_id', 'crmmsgcsitem_message', 'crmmsgclass_id', 'brand_id', 'site_id', 'crmmsgcs_id', '_createby', '_createdate', '_modifyby', '_modifydate' 
			], $where->sql);
			...
			$result->record = array_merge($record, [
				...
				'crmmsgclass_name' => \FGTA4\utils\SqlUtility::Lookup($record['crmmsgclass_id'], $this->db, 'mst_crmmsgclass', 'crmmsgclass_id', 'crmmsgclass_name'),
