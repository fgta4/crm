...
const tbl_list = $('#pnl_edittxgrid-tbl_list');
const txt_title = $('#pnl_edittxgrid-title');
const pnl_control = $('#pnl_edittxgrid-control');
const btn_removechecked  = $('#pnl_edittxgrid-removechecked');
const btn_addrow = $('#pnl_edittxgrid-addrow');
...
export function OpenDetil(data) {
	...
	var fn_listloading = async (options) => {
		options.api = `${global.modulefullname}/tx-list`
	...
	var fn_listloaded = async (result, options) => {
		...
		var detilform = $ui.getPages().ITEMS['pnl_edittxform'].handler.getForm()
...
function grd_list_rowclick(tr, ev) {
	...
	$ui.getPages().show('pnl_edittxform', () => {
		$ui.getPages().ITEMS['pnl_edittxform'].handler.open(record, trid, header_data)
	})	
}
		...
		yes: () => {
			grd_list.removechecked({
				OnRemoving : async (options) => {
					var apiurl = `${global.modulefullname}/tx-delete`
...
function btn_addrow_click() {
	$ui.getPages().show('pnl_edittxform', ()=>{
		$ui.getPages().ITEMS['pnl_edittxform'].handler.createnew(header_data)
	})	
}