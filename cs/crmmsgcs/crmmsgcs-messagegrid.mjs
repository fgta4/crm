...
const tbl_list = $('#pnl_editmessagegrid-tbl_list');
const txt_title = $('#pnl_editmessagegrid-title');
const pnl_control = $('#pnl_editmessagegrid-control');
const btn_removechecked  = $('#pnl_editmessagegrid-removechecked');
const btn_addrow = $('#pnl_editmessagegrid-addrow');
...
export function createnew(data, options) {
	...
	grd_list.clear();
	txt_title.html(data.crmmsgcs_name)
	header_data = data;
}
...
export function OpenDetil(data) {
	...
	var fn_listloading = async (options) => {
		options.api = `${global.modulefullname}/message-list`
		options.criteria['id'] = data.crmmsgcs_id
	}
	var fn_listloaded = async (result, options) => {
		...
		var detilform = $ui.getPages().ITEMS['pnl_editmessageform'].handler.getForm()
...
function grd_list_rowclick(tr, ev) {
	...
	$ui.getPages().show('pnl_editmessageform', () => {
		$ui.getPages().ITEMS['pnl_editmessageform'].handler.open(record, trid, header_data)
	})	
}
		...
		yes: () => {
			grd_list.removechecked({
				OnRemoving : async (options) => {
					var apiurl = `${global.modulefullname}/message-delete`
...
function btn_addrow_click() {
	$ui.getPages().show('pnl_editmessageform', ()=>{
		$ui.getPages().ITEMS['pnl_editmessageform'].handler.createnew(header_data)
	})	
}