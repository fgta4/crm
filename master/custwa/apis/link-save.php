...
if (is_file(__DIR__ .'/data-link-handler.php')) {
	require_once __DIR__ .'/data-link-handler.php';
}
...
/**
 * crm/master/custwa/apis/link-save.php
 *
 * ==========
 * Detil-Save
 * ==========
 * Menampilkan satu baris data/record sesuai PrimaryKey,
 * dari tabel link custwa (mst_custwalinkreq)
 *
...
$API = new class extends custwaBase {
	...
	public function execute($data, $options) {
		...
		$handlerclassname = "\\FGTA4\\apis\\custwa_linkHandler";
		if (class_exists($handlerclassname)) {
			$hnd = new custwa_linkHandler($data, $options);
			$hnd->caller = &$this;
			$hnd->db = &$this->db;
			$hnd->auth = $this->auth;
		...
		try {
			...
			$sqlUpdateField  = [
					...
					'message', 'voubatch_id', 'vou_id', 'crmevent_id',
					'code', 'slpart', 'data', 'result',
					'status', 'custwa_id'
			];
			...
			try {
				...
				$header_table = 'mst_custwa';
				$header_primarykey = 'custwa_id';
				$detil_primarykey = 'custwa_id';
				...
				// result
				$options->criteria = [
					"custwalinkreq_id" => $obj->custwalinkreq_id
				];
				...
				$sqlFieldList = [
					...
					'status' => 'A.`status`', 'custwa_id' => 'A.`custwa_id`', '_createby' => 'A.`_createby`', '_createdate' => 'A.`_createdate`',
