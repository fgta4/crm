...
/**
 * crm/master/custwa/apis/save.php
 *
 * ====
 * Save
 * ====
 * Menampilkan satu baris data/record sesuai PrimaryKey,
 * dari tabel header custwa (mst_custwa)
 *
...
$API = new class extends custwaBase {
	...
	public function execute($data, $options) {
		$event = 'on-save';
		$tablename = 'mst_custwa';
		$primarykey = 'custwa_id';
		$autoid = $options->autoid;
		$datastate = $data->_state;
		$userdata = $this->auth->session_get_user();

		$handlerclassname = "\\FGTA4\\apis\\custwa_headerHandler";
		$hnd = null;
		if (class_exists($handlerclassname)) {
			$hnd = new custwa_headerHandler($options);
			$hnd->caller = &$this;
			$hnd->db = &$this->db;
			$hnd->auth = $this->auth;
		...
		try {
			...
			if ($obj->custwa_gender=='') { $obj->custwa_gender = '--NULL--'; }
			...
			try {
				...
				// result
				$options->criteria = [
					"custwa_id" => $obj->custwa_id
				];

				$criteriaValues = [
					"custwa_id" => " custwa_id = :custwa_id "
				];
				...
				$sqlFieldList = [
					'custwa_id' => 'A.`custwa_id`', 'custwa_name' => 'A.`custwa_name`', 'custwa_gender' => 'A.`custwa_gender`', '_createby' => 'A.`_createby`',
					'_createby' => 'A.`_createby`', '_createdate' => 'A.`_createdate`', '_modifyby' => 'A.`_modifyby`', '_modifydate' => 'A.`_modifydate`'
				];
				$sqlFromTable = "mst_custwa A";
				$sqlWhere = $where->sql;
