...
import * as hnd from  './custwa-linkgrid-hnd.mjs'

const tbl_list = $('#pnl_editlinkgrid-tbl_list');
const txt_title = $('#pnl_editlinkgrid-title');
const pnl_control = $('.pnl_editlinkgrid-control');
const btn_removechecked  = $('#pnl_editlinkgrid-removechecked');
const btn_addrow = $('#pnl_editlinkgrid-addrow');
...
export function createnew(data, options) {
	...
	txt_title.html(data.custwa_name)
...
export function OpenDetil(data) {
	...
	var fn_listloading = async (options) => {
		options.api = `${global.modulefullname}/link-list`
		options.criteria['id'] = data.custwa_id
	...
	var fn_listloaded = async (result, options) => {
		...
		var detilform = $ui.getPages().ITEMS['pnl_editlinkform'].handler.getForm()
		...
		setTimeout(()=>{
			var checkcolumns = document.querySelectorAll('#pnl_editlinkgrid-tbl_list .rowcheck');
			...
			var selectbutton = document.getElementById('pnl_editlinkgrid-tbl_list-selectall-button');
			if (detilform.AllowRemoveRecord) {
...
function grd_list_rowclick(tr, ev) {
	...
	$ui.getPages().show('pnl_editlinkform', () => {
		$ui.getPages().ITEMS['pnl_editlinkform'].handler.open(record, trid, header_data)
	})	
}
		...
		yes: () => {
			grd_list.removechecked({
				OnRemoving : async (options) => {
					var apiurl = `${global.modulefullname}/link-delete`
...
function btn_addrow_click() {
	$ui.getPages().show('pnl_editlinkform', ()=>{
		$ui.getPages().ITEMS['pnl_editlinkform'].handler.createnew(header_data)
	})	
}