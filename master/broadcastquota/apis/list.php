...
/**
 * crm/master/broadcastquota/apis/list.php
 *
 * ========
 * DataList
 * ========
 * Menampilkan data-data pada tabel header broadcastquota (mst_broadcastquota)
	...
	public function execute($options) {
		...
		try {
			...
			$where = \FGTA4\utils\SqlUtility::BuildCriteria(
				$options->criteria,
				[
					"search" => " A.broadcastquota_id LIKE CONCAT('%', :search, '%') OR A.broadcastquota_name LIKE CONCAT('%', :search, '%') "
				]
			);
			...
			$stmt = $this->db->prepare("select count(*) as n from mst_broadcastquota A" . $where->sql);
			...
			$stmt = $this->db->prepare("
				select 
				A.broadcastquota_id, A.broadcasttype_id, A.broadcastquota_name, A.broadcastquota_saldovalue, A.broadcastquota_saldocredit, A.prepaid_accbudget_id, A.prepaid_coa_id, A.broadcastmodel_id, A._createby, A._createdate, A._modifyby, A._modifydate 
				from mst_broadcastquota A
			" . $where->sql . $limit);
			$stmt->execute($where->params);
