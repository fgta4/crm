...
import * as hnd from  './cust-contactgrid-hnd.mjs'

const tbl_list = $('#pnl_editcontactgrid-tbl_list');
const txt_title = $('#pnl_editcontactgrid-title');
const pnl_control = $('.pnl_editcontactgrid-control');
const btn_removechecked  = $('#pnl_editcontactgrid-removechecked');
const btn_addrow = $('#pnl_editcontactgrid-addrow');
...
export function OpenDetil(data) {
	...
	var fn_listloading = async (options) => {
		options.api = `${global.modulefullname}/contact-list`
	...
	var fn_listloaded = async (result, options) => {
		...
		var detilform = $ui.getPages().ITEMS['pnl_editcontactform'].handler.getForm()
...
function grd_list_rowclick(tr, ev) {
	...
	$ui.getPages().show('pnl_editcontactform', () => {
		$ui.getPages().ITEMS['pnl_editcontactform'].handler.open(record, trid, header_data)
	})	
}
		...
		yes: () => {
			grd_list.removechecked({
				OnRemoving : async (options) => {
					var apiurl = `${global.modulefullname}/contact-delete`
...
function btn_addrow_click() {
	$ui.getPages().show('pnl_editcontactform', ()=>{
		$ui.getPages().ITEMS['pnl_editcontactform'].handler.createnew(header_data)
	})	
}