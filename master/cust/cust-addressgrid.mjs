...
import * as hnd from  './cust-addressgrid-hnd.mjs'

const tbl_list = $('#pnl_editaddressgrid-tbl_list');
const txt_title = $('#pnl_editaddressgrid-title');
const pnl_control = $('.pnl_editaddressgrid-control');
const btn_removechecked  = $('#pnl_editaddressgrid-removechecked');
const btn_addrow = $('#pnl_editaddressgrid-addrow');
...
export function OpenDetil(data) {
	...
	var fn_listloading = async (options) => {
		options.api = `${global.modulefullname}/address-list`
		options.criteria['id'] = data.cust_id
	}
	var fn_listloaded = async (result, options) => {
		...
		var detilform = $ui.getPages().ITEMS['pnl_editaddressform'].handler.getForm()
...
function grd_list_rowclick(tr, ev) {
	...
	$ui.getPages().show('pnl_editaddressform', () => {
		$ui.getPages().ITEMS['pnl_editaddressform'].handler.open(record, trid, header_data)
	})	
}
		...
		yes: () => {
			grd_list.removechecked({
				OnRemoving : async (options) => {
					var apiurl = `${global.modulefullname}/address-delete`
...
function btn_addrow_click() {
	$ui.getPages().show('pnl_editaddressform', ()=>{
		$ui.getPages().ITEMS['pnl_editaddressform'].handler.createnew(header_data)
	})	
}