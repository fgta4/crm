...
import * as hnd from  './voubatch-tncform-hnd.mjs'

const reload_header_modified = true;

const txt_caption = $('#pnl_edittncform-caption')
const txt_title = $('#pnl_edittncform-title')
const btn_edit = $('#pnl_edittncform-btn_edit')
const btn_save = $('#pnl_edittncform-btn_save')
const btn_delete = $('#pnl_edittncform-btn_delete')
const btn_prev = $('#pnl_edittncform-btn_prev')
const btn_next = $('#pnl_edittncform-btn_next')
const btn_addnew = $('#pnl_edittncform-btn_addnew')
const chk_autoadd = $('#pnl_edittncform-autoadd')


const pnl_form = $('#pnl_edittncform-form')
const obj = {
	txt_voutnc_id: $('#pnl_edittncform-txt_voutnc_id'),
	txt_voutnc_order: $('#pnl_edittncform-txt_voutnc_order'),
	txt_voutnc_descr: $('#pnl_edittncform-txt_voutnc_descr'),
	txt_voubatch_id: $('#pnl_edittncform-txt_voubatch_id')
}
	...
	document.addEventListener('OnButtonBack', (ev) => {
		...
		if ($ui.getPages().getCurrentPage()==this_page_id) {
			...
			if (form.isDataChanged()) {
				form.canceledit(()=>{
					$ui.getPages().show('pnl_edittncgrid', ()=>{
						form.setViewMode()
						$ui.getPages().ITEMS['pnl_edittncgrid'].handler.scrolllast()
					})					
				})
			} else {
				$ui.getPages().show('pnl_edittncgrid', ()=>{
...
export function open(data, rowid, hdata) {
	...
	var fn_dataopening = async (options) => {
		options.api = `${global.modulefullname}/tnc-open`
...
async function form_datasaving(data, options) {
	options.api = `${global.modulefullname}/tnc-save`
...
async function form_datasaved(result, options) {
	...
	*/
	...
	form.rowid = $ui.getPages().ITEMS['pnl_edittncgrid'].handler.updategrid(data, form.rowid)
	var rowdata = {
		data: data,
		rowid: form.rowid
	}
...
async function form_deleting(data, options) {
	options.api = `${global.modulefullname}/tnc-delete`
...
async function form_deleted(result, options) {
	...
	$ui.getPages().show('pnl_edittncgrid', ()=>{
		$ui.getPages().ITEMS['pnl_edittncgrid'].handler.removerow(form.rowid)
	});
...
function btn_prev_click() {
	...
	var record = $ui.getPages().ITEMS['pnl_edittncgrid'].handler.getGrid().DATA[dataid]
