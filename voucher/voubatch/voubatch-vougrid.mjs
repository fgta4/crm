...
import * as hnd from  './voubatch-vougrid-hnd.mjs'

const tbl_list = $('#pnl_editvougrid-tbl_list');
const txt_title = $('#pnl_editvougrid-title');
const pnl_control = $('.pnl_editvougrid-control');
const btn_removechecked  = $('#pnl_editvougrid-removechecked');
const btn_addrow = $('#pnl_editvougrid-addrow');
...
export function OpenDetil(data) {
	...
	var fn_listloading = async (options) => {
		options.api = `${global.modulefullname}/vou-list`
	...
	var fn_listloaded = async (result, options) => {
		...
		var detilform = $ui.getPages().ITEMS['pnl_editvouform'].handler.getForm()
		...
		setTimeout(()=>{
			var checkcolumns = document.querySelectorAll('#pnl_editvougrid-tbl_list .rowcheck');
			...
			var selectbutton = document.getElementById('pnl_editvougrid-tbl_list-selectall-button');
			if (detilform.AllowRemoveRecord) {
...
function grd_list_rowclick(tr, ev) {
	...
	$ui.getPages().show('pnl_editvouform', () => {
		$ui.getPages().ITEMS['pnl_editvouform'].handler.open(record, trid, header_data)
	})	
}
		...
		yes: () => {
			grd_list.removechecked({
				OnRemoving : async (options) => {
					var apiurl = `${global.modulefullname}/vou-delete`
...
function btn_addrow_click() {
	$ui.getPages().show('pnl_editvouform', ()=>{
		$ui.getPages().ITEMS['pnl_editvouform'].handler.createnew(header_data)
	})	
}