...
import * as hnd from  './voubatch-tncgrid-hnd.mjs'

const tbl_list = $('#pnl_edittncgrid-tbl_list');
const txt_title = $('#pnl_edittncgrid-title');
const pnl_control = $('.pnl_edittncgrid-control');
const btn_removechecked  = $('#pnl_edittncgrid-removechecked');
const btn_addrow = $('#pnl_edittncgrid-addrow');
...
export function createnew(data, options) {
	...
	txt_title.html(data.voubatch_descr)
...
export function OpenDetil(data) {
	...
	var fn_listloading = async (options) => {
		options.api = `${global.modulefullname}/tnc-list`
		options.criteria['id'] = data.voubatch_id
	...
	var fn_listloaded = async (result, options) => {
		...
		var detilform = $ui.getPages().ITEMS['pnl_edittncform'].handler.getForm()
		...
		setTimeout(()=>{
			var checkcolumns = document.querySelectorAll('#pnl_edittncgrid-tbl_list .rowcheck');
			...
			var selectbutton = document.getElementById('pnl_edittncgrid-tbl_list-selectall-button');
			if (detilform.AllowRemoveRecord) {
...
function grd_list_rowclick(tr, ev) {
	...
	$ui.getPages().show('pnl_edittncform', () => {
		$ui.getPages().ITEMS['pnl_edittncform'].handler.open(record, trid, header_data)
	})	
}
		...
		yes: () => {
			grd_list.removechecked({
				OnRemoving : async (options) => {
					var apiurl = `${global.modulefullname}/tnc-delete`
...
function btn_addrow_click() {
	$ui.getPages().show('pnl_edittncform', ()=>{
		$ui.getPages().ITEMS['pnl_edittncform'].handler.createnew(header_data)
	})	
}