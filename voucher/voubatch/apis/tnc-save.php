...
/**
 * crm/voucher/voubatch/apis/tnc-save.php
 *
 * ==========
 * Detil-Save
 * ==========
 * Menampilkan satu baris data/record sesuai PrimaryKey,
 * dari tabel tnc voubatch (mst_voutnc)
 *
...
$API = new class extends voubatchBase {
	...
	public function execute($data, $options, $files) {
		...
		if (class_exists($handlerclassname)) {
			$hnd = new voubatch_tncHandler($data, $options);
			$hnd->caller = &$this;
			$hnd->db = &$this->db;
			$hnd->auth = $this->auth;
		...
		try {
			...
			$sqlUpdateField  = [
					'voutnc_id', 'voutnc_order', 'voutnc_descr', 'voubatch_id'
			];
			...
			try {
				...
				// result
				$options->criteria = [
					"voutnc_id" => $obj->voutnc_id
				];

				$criteriaValues = [
					"voutnc_id" => " voutnc_id = :voutnc_id "
				];
				...
				$sqlFieldList = [
					'voutnc_id' => 'A.`voutnc_id`', 'voutnc_order' => 'A.`voutnc_order`', 'voutnc_descr' => 'A.`voutnc_descr`', 'voubatch_id' => 'A.`voubatch_id`',
					'_createby' => 'A.`_createby`', '_createdate' => 'A.`_createdate`', '_modifyby' => 'A.`_modifyby`', '_modifydate' => 'A.`_modifydate`'
				];
				$sqlFromTable = "mst_voutnc A";
				$sqlWhere = $where->sql;
