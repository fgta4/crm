<?php
namespace FGTA4\apis;

require_once __DIR__ . '/code128dimension.php';
...
class Code128Barcode
{
    ...
    public static function generate(string $code, int $density = 1)
    {
		...
		$image_height = $height + 35;

        // create a true color image at the specified height and width
        $imageframe = imagecreatetruecolor($image_width, $image_height);
		imagefill($imageframe, 0, 0, self::getWhiteColor($imageframe));


        // create a true color image at the specified height and width
        $image = imagecreatetruecolor($width, $height+30);
		...
		imagestring($image, 4, $text_x, $text_y, $code, $color);		
        ...
        for ($i = 0, $x = 0, $inc = round(($density / 72) * 100); $i < strlen($enc_str); $i++) {
            ...
            for ($n = 0; $n < $val; $n++, $x += $inc) {
                if ($i % 2 == 0) {
                    imageline($image, $x, 0, $x, $dimensions->height, $black);
                }
            }
		...
		imagecopymerge($imageframe, $image, 5, 5, 0, 0, $width, $height+30, 100);
     ...
     * @return Dimensions
     */
    private static function getDimensions($code, $density)
    {
		...
		$barcode_width = round($width * 72);
		$barcode_height = round($height * 72);
