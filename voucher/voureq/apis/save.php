...
/**
 * crm/voucher/voureq/apis/save.php
 *
 * ====
 * Save
 * ====
 * Menampilkan satu baris data/record sesuai PrimaryKey,
 * dari tabel header voureq (mst_voureq)
 *
 ...
 * tanggal 11/10/2023
 */
$API = new class extends voureqBase {
	
	public function execute($data, $options) {
		$event = 'on-save';
		$tablename = 'mst_voureq';
		$primarykey = 'voureq_id';
		$autoid = $options->autoid;
		$datastate = $data->_state;
		$userdata = $this->auth->session_get_user();

		$handlerclassname = "\\FGTA4\\apis\\voureq_headerHandler";
		$hnd = null;
		if (class_exists($handlerclassname)) {
			$hnd = new voureq_headerHandler($options);
			$hnd->caller = &$this;
			$hnd->db = &$this->db;
			$hnd->auth = $this->auth;
		...
		try {
			...
			unset($obj->voureq_isvougenerate);
			unset($obj->voureq_isresend);
			...
			try {
				...
				// result
				$options->criteria = [
					"voureq_id" => $obj->voureq_id
				];

				$criteriaValues = [
					"voureq_id" => " voureq_id = :voureq_id "
				];
				...
				$sqlFieldList = [
					'voureq_id' => 'A.`voureq_id`', 'voureq_room' => 'A.`voureq_room`', 'voureq_phone' => 'A.`voureq_phone`', 'voureq_message' => 'A.`voureq_message`',
					'voureq_isvougenerate' => 'A.`voureq_isvougenerate`', 'voureq_result' => 'A.`voureq_result`', 'vou_id' => 'A.`vou_id`', 'site_id' => 'A.`site_id`',
					'voureq_info' => 'A.`voureq_info`', 'voureq_isresend' => 'A.`voureq_isresend`', '_createby' => 'A.`_createby`', '_createdate' => 'A.`_createdate`',
					'_createby' => 'A.`_createby`', '_createdate' => 'A.`_createdate`', '_modifyby' => 'A.`_modifyby`', '_modifydate' => 'A.`_modifydate`'
				];
				$sqlFromTable = "mst_voureq A";
				$sqlWhere = $where->sql;
