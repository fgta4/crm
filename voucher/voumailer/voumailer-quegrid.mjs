...
import * as hnd from  './voumailer-quegrid-hnd.mjs'

const tbl_list = $('#pnl_editquegrid-tbl_list');
const txt_title = $('#pnl_editquegrid-title');
const pnl_control = $('.pnl_editquegrid-control');
const btn_removechecked  = $('#pnl_editquegrid-removechecked');
const btn_addrow = $('#pnl_editquegrid-addrow');
...
export function OpenDetil(data) {
	...
	var fn_listloading = async (options) => {
		options.api = `${global.modulefullname}/que-list`
	...
	var fn_listloaded = async (result, options) => {
		...
		var detilform = $ui.getPages().ITEMS['pnl_editqueform'].handler.getForm()
...
function grd_list_rowclick(tr, ev) {
	...
	$ui.getPages().show('pnl_editqueform', () => {
		$ui.getPages().ITEMS['pnl_editqueform'].handler.open(record, trid, header_data)
	})	
}
		...
		yes: () => {
			grd_list.removechecked({
				OnRemoving : async (options) => {
					var apiurl = `${global.modulefullname}/que-delete`
...
function btn_addrow_click() {
	$ui.getPages().show('pnl_editqueform', ()=>{
		$ui.getPages().ITEMS['pnl_editqueform'].handler.createnew(header_data)
	})	
}