...
/**
 * crm/voucher/voumailer/apis/que-save.php
 *
 * ==========
 * Detil-Save
 * ==========
...
$API = new class extends voumailerBase {
	...
	public function execute($data, $options) {
		...
		if (class_exists($handlerclassname)) {
			$hnd = new voumailer_queHandler($data, $options);
			$hnd->caller = &$this;
			$hnd->db = &$this->db;
			$hnd->auth = $this->auth;
		...
		try {
			...
			if ($obj->msg_id=='') { $obj->msg_id = '--NULL--'; }



			unset($obj->voumailerque_issend);
			unset($obj->voumailerque_sendby);
			unset($obj->voumailerque_senddate);
			...
			try {
				...
				// result
				$options->criteria = [
					"voumailerque_id" => $obj->voumailerque_id
				];
