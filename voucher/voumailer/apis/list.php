...
/**
 * crm/voucher/voumailer/apis/list.php
 *
 * ========
 * DataList
 * ========
 * Menampilkan data-data pada tabel header voumailer (mst_voumailer)
...
$API = new class extends voumailerBase {
	...
	public function execute($options) {
		...
		try {
			...
			$criteriaValues = [
				"search" => " A.voumailer_id LIKE CONCAT('%', :search, '%') "
			];
			if (is_object($hnd)) {
			...
			foreach ($rows as $row) {
				...
				*/
				...
				$this->addFields('undefined', 'voumailer_commitby', $record, $GLOBALS['MAIN_USERTABLE'], 'user_fullname', 'user_id');
