...
import * as hnd from  './voumailer-emailgrid-hnd.mjs'

const tbl_list = $('#pnl_editemailgrid-tbl_list');
const txt_title = $('#pnl_editemailgrid-title');
const pnl_control = $('.pnl_editemailgrid-control');
const btn_removechecked  = $('#pnl_editemailgrid-removechecked');
const btn_addrow = $('#pnl_editemailgrid-addrow');
...
export function createnew(data, options) {
	...
	grd_list.clear();
	txt_title.html(data.voumailer_descr)
	header_data = data;
}
...
export function OpenDetil(data) {
	...
	var fn_listloading = async (options) => {
		options.api = `${global.modulefullname}/email-list`
		options.criteria['id'] = data.voumailer_id
	...
	var fn_listloaded = async (result, options) => {
		...
		var detilform = $ui.getPages().ITEMS['pnl_editemailform'].handler.getForm()
...
function grd_list_rowclick(tr, ev) {
	...
	$ui.getPages().show('pnl_editemailform', () => {
		$ui.getPages().ITEMS['pnl_editemailform'].handler.open(record, trid, header_data)
	})	
}
		...
		yes: () => {
			grd_list.removechecked({
				OnRemoving : async (options) => {
					var apiurl = `${global.modulefullname}/email-delete`
...
function btn_addrow_click() {
	$ui.getPages().show('pnl_editemailform', ()=>{
		$ui.getPages().ITEMS['pnl_editemailform'].handler.createnew(header_data)
	})	
}